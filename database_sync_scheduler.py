...
def _copy_database(source: Path, target: Path) -> None:
  " " ""Copy source database file to target using sqlite3 backup AP"I".""
    try:
...
def synchronize_databases(master: Path, replicas: Iterable[Path]) -> None:
  ...
  " " ""Return database names listed in the documentation fil"e".""
    names: list[str] = [
...
if __name__ == "__main__":
    ...
    replica_dbs = [
        name for name in db_names if name != "production.db"]
