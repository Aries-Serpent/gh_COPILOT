...
def apply_manual_fixes():
    ...
    'f"External Backup Root: {self.external_backup_root}""', 'f"External Backup Root: {self.external_backup_root}"'),
        (
    'f"Target Success Rate: >{self.success_target}% (
    Comprehensive Standard)""', 'f"Target Success Rate: >{self.success_target}% (
    ...
    if file_path.exists():
        try:
            ...
            if len(lines) > 57:  # line 58 (0-indexed)
                line = lines[57]
                if '(
    ' in line and not line.strip().endswith(',') and not line.strip().endswith(')'):
