...
def generate_final_validation_report():
    ...
    required_tables = ['code_variants', 'deployment_findings', 'historical_tracking']
    missing_tables = [t for t in required_tables if t not in tables]
    
    validation_results["capture_completeness"]["tables_present"] = len(required_tables) - len(missing_tables)
    ...
    print(f"[SUCCESS] Required tables present: {len(required_tables) - len(missing_tables)}/{len(required_tables)}")
    ...
    cursor.execute("SELECT COUNT(*) FROM code_variants WHERE phase_type = 'V3_ADVANCED'")
    v3_advanced_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM code_variants WHERE phase_type = '7_PHASE'")
    seven_phase_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM code_variants WHERE phase_type = '5_PHASE_ORIGINAL'")
    five_phase_count = cursor.fetchone()[0]
    
    validation_results["code_variants_analysis"] = {
        "total_variants": total_variants,
        "functional_variants": functional_variants,
        "v3_advanced_count": v3_advanced_count,
        "seven_phase_count": seven_phase_count,
        "five_phase_count": five_phase_count,
        "functionality_rate": functional_variants / total_variants if total_variants > 0 else 0
    }
    ...
    print(f"[SUCCESS] Functionality rate: {functional_variants/total_variants*100:.1f}%")
    ...
    cursor.execute("SELECT COUNT(*) FROM deployment_findings WHERE result_status = 'SUCCESS'")
    successful_findings = cursor.fetchone()[0]
    
    cursor.execute("SELECT finding_type, COUNT(*) FROM deployment_findings GROUP BY finding_type")
    findings_by_type = cursor.fetchall()
    
    validation_results["findings_analysis"] = {
        "total_findings": total_findings,
        "successful_findings": successful_findings,
        "success_rate": successful_findings / total_findings if total_findings > 0 else 0,
    ...
    print(f"[SUCCESS] Success rate: {successful_findings/total_findings*100:.1f}%")
    ...
    cursor.execute("SELECT COUNT(*) FROM historical_tracking WHERE status = 'COMPLETED'")
    ...
    print(f"[SUCCESS] Compliance status: {'COMPLIANT' if compliance_score >= 95 else 'NON_COMPLIANT'}")
    ...
    print(f"[SUCCESS] Database operational: {'YES' if final_validation['database_operational'] else 'NO'}")
    print(f"[SUCCESS] Code captured: {'YES' if final_validation['code_captured'] else 'NO'}")
    print(f"[SUCCESS] Findings captured: {'YES' if final_validation['findings_captured'] else 'NO'}")
    print(f"[SUCCESS] Functional code available: {'YES' if final_validation['functional_code_available'] else 'NO'}")
    print(f"[SUCCESS] V3 Advanced available: {'YES' if final_validation['v3_advanced_available'] else 'NO'}")
    print(f"[SUCCESS] Compliance satisfied: {'YES' if final_validation['compliance_satisfied'] else 'NO'}")
    ...
    if all_validations_passed:
        ...
        validation_results["recommendations"] = [
            "All code variants are functional - no replacement needed",
            "V3_ADVANCED variant is the most complete implementation",
            "Keep all variants as historical references",
            "Database contains complete audit trail for enterprise compliance",
        ...
        validation_results["recommendations"] = [
            "Review capture process for missing components",
            "Do not proceed with code replacement until validation passes",
            "Ensure all required tables are present",
            "Verify code variants are properly captured"
        ]
