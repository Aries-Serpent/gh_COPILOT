...
class DatabaseCleanupExecutor:
    ...
    def create_backup(self, staging_path: Path) -> Path:
        ...
        try:
            ...
            print(
                f"[BAR_CHART] Backup size: {self._get_directory_size(backup_path):.2f} MB")
            return backup_path
    ...
    def load_cleanup_plan(self, analysis_file: str) -> dict:
        ...
        try:
            ...
            print(
                f"[BAR_CHART] Files to remove: {cleanup_plan['total_files']}")
            print(
                f"[BAR_CHART] Estimated space savings: {cleanup_plan['estimated_savings']:.2f} MB")
            ...
            for file_info in identical_files:
                ...
                try:
                    ...
                    if expected_hash == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855":
                        ...
                        self.errors.append(]
                            f"Hash mismatch, skipping: {staging_path}")
                        pbar.update(1)
                        continue
                    ...
                    pbar.set_description(]
                        f"[TRASH] Removed: {staging_path.name}")
    ...
    def execute_full_cleanup(self, analysis_file: str) -> bool:
        ...
        try:
            ...
            print(
                f"[SUCCESS] Files removed: {cleanup_results['files_removed']}")
            print(
                f"[SUCCESS] Space saved: {cleanup_results['space_saved_mb']:.2f} MB")
            print(f"[SUCCESS] Backup location: {backup_path}")
            print(f"[SUCCESS] Report: {report_path}")

            if self.errors:
                print(
                    f"[WARNING] {len(self.errors)} errors occurred (see report for details)")
...
def main():
    ...
    analysis_files = list(Path('.').glob(]
        'DATABASE_REDUNDANCY_ANALYSIS_*.json'))
    if not analysis_files:
        print(
            "[ERROR] No analysis file found. Run DATABASE_REDUNDANCY_ANALYZER.py first.")
        return False
