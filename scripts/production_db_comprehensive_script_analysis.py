...
class ProductionDBScriptAnalyzer:
    ...
    def scan_filesystem_scripts(self):
        ...
        for py_file in self.workspace_path.rglob("*.py"):
            try:
                ...
                if any(part.startswith(('_backup', '__pycache__', '.git'))
    ...
    def analyze_database_scripts(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='file_system_mapping'")
    ...
    def analyze_adaptive_generation_capability(self):
        ...
        try:
            ...
            generation_tables = [
                keyword in t.lower() for keyword in ['generation', 'template', 'pattern'])]
    ...
    def generate_recommendations(self):
        ...
        fs_scripts = self.results.get("filesystem_scripts", {})
        framework_scripts = [
        ) if data.get("contains_framework_logic")]
    ...
    def _save_results(self):
        ...
        json_file = self.workspace_path / \
            f"PRODUCTION_DB_SCRIPT_ANALYSIS_{timestamp}.json"
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(self.results, f, indent=2, ensure_ascii=False)

        # Save markdown report
        md_file = self.workspace_path / \
            f"PRODUCTION_DB_SCRIPT_ANALYSIS_{timestamp}.md"
    ...
    def _write_markdown_report(self, f):
        ...
        f.write(]
            f"Answer: **{'YES' if key_q.get('all_scripts_stored_in_db') else 'NO'}**\n")
        f.write(]
            f"Explanation: {key_q.get('explanation_script_storage', 'N/A')}\n\n")

        f.write(f"**Q2: Can the database generate environment-adaptive scripts?**\n")
        f.write(]
            f"Answer: **{'YES' if key_q.get('can_generate_adaptive_scripts') else 'NO'}**\n")
        f.write(]
            f"Explanation: {key_q.get('explanation_adaptive_capability', 'N/A')}\n\n")

        # Metrics
        metrics = summary.get("metrics", {})
        f.write("### Key Metrics\n\n")
        f.write(]
            f"- **Filesystem Scripts:** {metrics.get('filesystem_scripts', 0)}\n")
        f.write(]
            f"- **Database Scripts:** {metrics.get('database_scripts', 0)}\n")
        f.write(]
            f"- **Synchronized Scripts:** {metrics.get('synchronized_scripts', 0)}\n")
        f.write(]
            f"- **Environment-Aware Scripts:** {metrics.get('environment_aware_scripts', 0)}\n")
        f.write(]
            f"- **Template Scripts:** {metrics.get('template_scripts', 0)}\n\n")

        # Detailed comparison
        f.write("## Script Comparison Details\n\n")
        f.write(]
            f"- **Total in Filesystem:** {comparison.get('total_filesystem', 0)}\n")
        f.write(]
            f"- **Total in Database:** {comparison.get('total_database', 0)}\n")
        f.write(f"- **In Both Locations:** {comparison.get('in_both', 0)}\n")
        f.write(]
            f"- **Only in Filesystem:** {comparison.get('only_filesystem', 0)}\n")
        f.write(]
            f"- **Only in Database:** {comparison.get('only_database', 0)}\n")
        f.write(]
            f"- **Content Matches:** {comparison.get('content_matches', 0)}\n")
        f.write(]
            f"- **Content Differs:** {comparison.get('content_differs', 0)}\n\n")
        ...
        if recommendations:
            ...
            for i, rec in enumerate(recommendations, 1):
                f.write(]
                    f"### {i}. {rec.get('type', 'Unknown')} ({rec.get('priority', 'UNKNOWN')} Priority)\n")
