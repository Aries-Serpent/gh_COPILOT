...
def validate_root_organization():
    ...
    for intended_file in intended_root_files:
        if intended_file not in actual_root_files:
            missing_files.append(
                {"file": intended_file, "description": intended_root_files[intended_file], "status": "MISSING"}
            )
    ...
    for dir_name in expected_directories:
        ...
        if dir_path.exists() and dir_path.is_dir():
            ...
            present_directories.append({"directory": dir_name, "file_count": file_count, "status": "PRESENT"})
            print(f"✅ {dir_name}/: {file_count} files")
        else:
            missing_directories.append({"directory": dir_name, "status": "MISSING"})
    ...
    if misplaced_files:
        ...
        config_files = [
            f for f in misplaced_files if f["file"].endswith(".json") and f["file"] != "COPILOT_NAVIGATION_MAP.json"
        ]
        if config_files:
    ...
    if missing_directories:
        recommendations.append(
            f"📁 Create missing directories: {', '.join([d['directory'] for d in missing_directories])}"
        )
