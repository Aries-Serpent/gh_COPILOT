...
class BatchConsolidationProcessor:
    ...
    def load_consolidation_opportunities(self) -> List[Dict[str, Any]]:
        ...
        if not analysis_files:
            raise FileNotFoundError("No script modulation analysis results found. Run script_modulation_analyzer.py first.")
    ...
    def assess_consolidation_risk(self, opportunity: Dict[str, Any]) -> str:
        ...
        if any(keyword in opportunity["consolidation_type"] for keyword in ["optimization", "enterprise", "comprehensive"]):
    ...
    def finalize_batch(self, batch_id: int, opportunities: List[Dict[str, Any]], risk_levels: List[str]) -> ConsolidationBatch:
    ...
    def execute_single_consolidation(self, opportunity: Dict[str, Any], batch_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create backup
            backup_dir = self.backup_root / f"{batch_id}_{Path(primary_script).stem}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def create_safety_checkpoint(self, batch_number: int) -> Dict[str, Any]:
        ...
        # Save checkpoint
        checkpoint_file = self.reports_dir / f"safety_checkpoint_{batch_number:03d}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run_batch_consolidation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"📈 Success Rate: {(self.batch_results['successful_consolidations'] / max(self.batch_results['total_opportunities_processed'], 1)) * 100:.1f}%")
