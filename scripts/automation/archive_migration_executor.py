...
class ArchiveMigrationExecutor:
    def __init__(self):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(self.logs_folder / "archive_migration.log"), logging.StreamHandler()],
        )
        ...
        for file_info in files_to_migrate:
            ...
            try:
                ...
                migration_results["migrated_files"].append(
                    {"filename": file_info["filename"], "source": str(source_path), "destination": str(archive_path)}
                )
    ...
    def generate_migration_report(self, migration_results: Dict) -> Dict:
        ...
        report = {
            ...
            "migration_summary": {
                ...
                "success_rate": (migration_results["successful_migrations"] / migration_results["total_files"] * 100)
        ...
        report_path = (
            self.workspace_root
            / "reports"
            / f"archive_migration_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
