...
class CompleteRootMaintenanceValidator:
    ...
    def validate_complete_compliance(self) -> ComplianceResult:
        ...
        result = ComplianceResult(session_id=self.session_id, validation_time=datetime.now())
    ...
    def _discover_all_files(self, pbar: tqdm) -> List[Path]:
        ...
        skip_folders = {
            ...
            ".venv",
            "node_modules",
            Path(os.getenv("GH_COPILOT_BACKUP_ROOT", "/tmp")).name,
    ...
    def generate_compliance_report(self, result: ComplianceResult) -> str:
        ...
        report_lines.extend(["", "## 🚨 Compliance Violations", ""])
        ...
        for severity in ["HIGH", "MEDIUM", "LOW"]:
            if severity in violations_by_severity:
                ...
                report_lines.extend([f"### {icon} {severity} Priority ({len(violations)} violations)", ""])
