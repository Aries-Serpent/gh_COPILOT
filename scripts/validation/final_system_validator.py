...
class FinalSystemValidator:
    ...
    def _validate_file_system_integrity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "details": {"critical_directories": critical_dirs, "all_directories_accessible": success},
        }
    ...
    def _validate_configuration_consolidation(self) -> Dict[str, Any]:
        ...
        for location in [self.workspace_path, self.reports_path]:
            if location.exists():
                orphaned = [
                    f for f in location.glob("*.json") if "config" in f.name.lower() and f.parent != self.config_path
                ]
                if orphaned:
                    config_analysis["orphaned_configs"].extend([str(f) for f in orphaned])

        consolidation_success = (
            config_analysis["config_directory_exists"]
            and config_analysis["total_config_files"] > 0
            and len(config_analysis["orphaned_configs"]) == 0
        )
    ...
    def _validate_python_scripts(self) -> Dict[str, Any]:
        ...
        validation_success = (
            script_validation["accessible_scripts"] > 0 and len(script_validation["failed_scripts"]) == 0
        )
    ...
    def _validate_dependencies(self) -> Dict[str, Any]:
        ...
        dependency_analysis = {"critical_imports_available": True, "missing_dependencies": [], "import_tests": {}}
    ...
    def _validate_database_integrity(self) -> Dict[str, Any]:
        ...
        integrity_success = database_analysis["databases_found"] > 0 and database_analysis["accessible_databases"] > 0
        ...
        self.validation_results.update(
            {
                ...
                "validation_summary": {
                    ...
                    "successful_phases": sum(
                        1 for r in self.validation_results["validation_phases"].values() if r.get("success", False)
                    ),
                    "failed_phases": sum(
                        1 for r in self.validation_results["validation_phases"].values() if not r.get("success", False)
                    ),
    ...
    def _log_completion_summary(self, report_path: str):
        ...
        print(
            f"Enterprise Compliance: {'✅ ACHIEVED' if self.validation_results['enterprise_compliance'] else '❌ NOT MET'}"
        )
