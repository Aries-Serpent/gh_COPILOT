...
"""
        ...
        for category, results in test_results["category_results"].items():
            functionality_rate = (results["functional_count"] / results["scripts_tested"] * 100) if results["scripts_tested"] > 0 else 0
...
def main():
    ...
    try:
        ...
        logger.info(f"Functionality Rate: {((test_results['fully_functional'] + test_results['basic_functional'])/test_results['total_scripts_tested']*100):.1f}%")
