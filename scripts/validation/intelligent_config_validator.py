...
class IntelligentConfigReferenceValidator:
    ...
    def is_valid_config_reference(self, value: str) -> bool:
        ...
        config_indicators = [".json", ".yaml", ".yml", ".ini", ".cfg", ".conf"]
    ...
    def generate_intelligent_report(self, validation_results: Dict[str, Any]) -> str:
        ...
        report = {
            "intelligent_config_validation": {
                ...
                "success_metrics": {
                    ...
                    if validation_results["overall_success_rate"] >= 80.0
                    and validation_results.get("config_accessibility_rate", 100) >= 95.0
                    else "NEEDS_REVIEW",
                ...
                "recommendations": {
                    "next_phase": "Proceed to Phase 3 Final System Validation"
                    if validation_results["overall_success_rate"] >= 80.0
                    else "Address remaining accessibility issues",
        ...
        report_file = (
            self.reports_folder / f"intelligent_config_validation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
            ...
            if script_detail.get("status") == "NEEDS_ATTENTION":
                ...
                if inaccessible_count > 0:
                    recommendations.append(
                        f"Review {inaccessible_count} config references in {script_name} - may need report files"
                    )
...
def main():
    ...
    try:
        ...
        print(
            f"✅ Mission Status: {'SUCCESS' if overall_success >= 80.0 and config_success >= 95.0 else 'NEEDS_REVIEW'}"
        )
