    ...
    def get_repository_scripts(self) -> Dict[str, Dict[str, str]]:
        ...
        for file_path in self.workspace_root.rglob('*'):
            if (file_path.is_file() and 
                ...
                'archives' not in file_path.parts):
            ...
            with sqlite3.connect(self.script_db_path) as conn:
                ...
                for script_path in scripts_to_update:
                    ...
                    if existing:
                        ...
                        while True:
                            cursor.execute("SELECT template_id FROM script_templates WHERE template_id = ?", (template_id,))
                        ...
                        cursor.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            template_id, script_name, "repository_script", 
                            f"Auto-synced from {script_path}", content, 
                            script_data['hash'], current_time, current_time
                        ))
    ...
    def generate_validation_report(self, output_file: Optional[str] = None) -> str:
        ...
        if results['missing_from_db']:
            ...
            for item in results['missing_from_db'][:10]:  # Limit for readability
                report_lines.append(f"| {item['script']} | {item['hash'][:16]}... | {item['size']} | {item['modified']} |")
