                    ...
                    for table in tables:
                        if any(keyword in table.lower() for keyword in ["template", "script", "content"]):
                            try:
                                ...
                                logging.warning("Table scan failed for %s: %s", table, exc)
    ...
    for i, script_path in enumerate(test_scripts):
        try:
            ...
            try:
                ...
                logging.warning("Syntax error in %s: %s", script_path.name, exc)
            ...
            try:
                ...
                result = subprocess.run(
                    ["flake8", "--select=E,W", temp_file_path], capture_output=True, text=True, timeout=5
                )
                ...
                if "temp_file_path" in locals() and os.path.exists(temp_file_path):
                    os.unlink(temp_file_path)
                logging.warning("Flake8 check failed for %s: %s", script_path.name, exc)
