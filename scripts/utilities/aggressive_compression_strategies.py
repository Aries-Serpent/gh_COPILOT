        ...
        for key, value in data.items():
            if isinstance(value, list) and len(value) > 100:
                ...
                if value and isinstance(value[0], dict):
                    ...
                    while len(sampled_items) < sample_size and sum(len(g) for g in groups.values()) > len(
                        sampled_items
                    ):
    ...
    def execute_aggressive_compression(self, data):
        ...
        with tqdm(total=100, desc="🔥 Aggressive compression", unit="%") as pbar:
            ...
            try:
                ...
                strategies["hybrid"] = {"path": hybrid_output, "size_mb": hybrid_output.stat().st_size / (1024 * 1024)}
            except Exception as e:
...
def apply_aggressive_compression(source_file, target_size_mb=99.0):
    ...
    report_path = (
        Path(source_file).parent / f"aggressive_compression_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
