...
class UnifiedWrapUpOrchestrator:
    ...
    def _initialize_root_maintainer(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'essential_files': [
                ...
                'package.json', 'Makefile', 'docker-compose.yml',
                'Dockerfile', 'pyproject.toml', 'COPILOT_NAVIGATION_MAP.json'
            ],
            'cleanup_candidates': [],
            'maintenance_actions': []
        }
    ...
    def _analyze_script_modularization(self, script_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            should_modularize = (
                size_kb > 5 or  # Large scripts
                len(lines) > 100 or  # Long scripts
                script_file.name.startswith(('temp_', 'test_', 'debug_')) or  # Temporary scripts
                category in ['analysis', 'automation', 'validation', 'database']  # Specific categories
            )
    ...
    def _get_modularization_reason(self, should_modularize: bool, size_kb: float, 
                                 lines: int, category: str) -> str:
...
"""
    ...
    def _format_config_validation_results(self, config_results: Dict[str, Any]) -> str:
        ...
        results.append(f"- **Functionality Preserved**: {'✅ Yes' if config_results['functionality_preserved'] else '❌ No'}")
