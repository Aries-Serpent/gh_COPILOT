...
class ProductionDatabaseValidator:
    ...
    def _validate_environment_safety(self):
        ...
        if violations:
            raise RuntimeError(f"[ALERT] CRITICAL: Recursive violations found: {violations}")
    ...
    def compare_databases(self) -> dict:
        ...
        if not root_analysis.get("is_valid_db") or not staging_analysis.get("is_valid_db"):
            return {
                "can_compare": False,
                "reason": "One or both databases are invalid"
            }
        
        # File-level comparison
        files_identical = root_analysis["file_hash"] == staging_analysis["file_hash"]
        size_difference = abs(root_analysis["file_size"] - staging_analysis["file_size"])
        ...
        print(f"   - Size difference: {comparison_results['size_difference_mb']:.2f} MB")
    ...
    def generate_recommendations(self) -> list:
        ...
        if not comparison.get("can_compare", False):
            recommendations.append("[ERROR] Cannot compare databases - analysis incomplete")
            return recommendations
        
        # If files are identical
        if comparison["files_identical"]:
            recommendations.append("[SUCCESS] SAFE TO CONSOLIDATE: Files are identical")
            recommendations.append("[PROCESSING] Recommended action: Keep root database, remove staging database")
            recommendations.append("[CLIPBOARD] Backup staging database before removal (enterprise compliance)")
            
        # If files are different
        else:
            if comparison["size_difference_mb"] < 1:
                recommendations.append("[WARNING] MINOR DIFFERENCES: Databases are similar but not identical")
            else:
                recommendations.append("[ALERT] SIGNIFICANT DIFFERENCES: Databases have substantial differences")
            
            # Structure analysis
            if comparison["structure_comparison"]["table_structure_identical"]:
                recommendations.append("[SUCCESS] Table structures are identical")
            else:
                recommendations.append("[WARNING] Table structures differ - manual review required")
            
            # Record differences
            if comparison["total_record_difference"] == 0:
                recommendations.append("[SUCCESS] All tables have identical record counts")
            else:
                recommendations.append(f"[WARNING] Total record difference: {comparison['total_record_difference']}")
        ...
        if root_analysis.get("exists") and staging_analysis.get("exists"):
            ...
            staging_time = datetime.fromisoformat(staging_analysis["modified_time"])
            
            if root_time > staging_time:
                recommendations.append("[?] Root database is newer - likely the active version")
            elif staging_time > root_time:
                recommendations.append("[?] Staging database is newer - may need to replace root")
            else:
                recommendations.append("[?] Both databases have identical modification times")
    ...
    def create_consolidation_plan(self) -> dict:
        ...
        consolidation_plan = {
            ...
            "database_to_remove": remove_db,
            "backup_required": True,
            "backup_location": f"E:/temp/gh_COPILOT_Backups/production_db_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db",
            "steps": [
                "1. Create backup of database to be removed",
                "2. Validate backup integrity",
                "3. Remove redundant database",
                "4. Update any references to removed database path",
                "5. Verify primary database functionality"
            ],
    ...
    def run_validation(self):
        ...
        with tqdm(total=100, desc="[SEARCH] Validation Progress", unit="%") as pbar:
            ...
            self.validation_results["root_db_analysis"] = self.analyze_database_file(self.root_db, "ROOT DATABASE")
            pbar.update(30)
            
            # Analyze staging database
            pbar.set_description("[BAR_CHART] Analyzing staging database")
            self.validation_results["staging_db_analysis"] = self.analyze_database_file(self.staging_db, "STAGING DATABASE")
            pbar.update(30)
    ...
    def _display_summary(self):
        ...
        # Database status
        print(f"Root Database: {'[SUCCESS] EXISTS' if root_analysis.get('exists') else '[ERROR] MISSING'}")
        ...
        print(f"Staging Database: {'[SUCCESS] EXISTS' if staging_analysis.get('exists') else '[ERROR] MISSING'}")
        ...
        if comparison.get("can_compare"):
            ...
            print(f"Files Identical: {'[SUCCESS] YES' if comparison['files_identical'] else '[ERROR] NO'}")
            print(f"Size Difference: {comparison['size_difference_mb']:.2f} MB")
        ...
        if plan.get("plan_type"):
            ...
            print(f"Space Savings: {plan['estimated_space_savings_mb']:.2f} MB")
