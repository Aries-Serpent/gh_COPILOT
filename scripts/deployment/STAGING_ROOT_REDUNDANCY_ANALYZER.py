...
class StagingRootRedundancyAnalyzer:
    ...
    def extract_base_name(self, filename: str) -> str:
        ...
        for suffix in ['_clean', '_enhanced', '_complete', '_final', '_v2', '_v3', '_advanced']:
    ...
    def analyze_file_similarity(self, file1: Dict, file2: Dict) -> float:
        ...
        try:
            ...
            func_similarity = len(func1 & func2) / len(func1 | func2) if len(func1 | func2) > 0 else 0
            
            # Class similarity
            class1 = set(file1.get("classes", []))
            class2 = set(file2.get("classes", []))
            class_similarity = len(class1 & class2) / len(class1 | class2) if len(class1 | class2) > 0 else 0
            ...
            # Overall similarity
            similarity = (size_ratio * 0.2 + func_similarity * 0.4 + class_similarity * 0.2 + import_similarity * 0.2)
    ...
    def identify_redundant_files(self, groups: Dict) -> List[Dict]:
        ...
        for group_name, files in groups.items():
            ...
            files.sort(key=lambda x: x.get("modification_time", 0), reverse=True)
            ...
            for i, file1 in enumerate(files):
                for j, file2 in enumerate(files[i+1:], i+1):
                    ...
                    if similarity > 0.8:  # High similarity threshold
                        ...
                        print(f"  [?] REDUNDANT: {file2['file_name']} (similarity: {similarity:.1%})")
                        ...
                        print(f"  [?] VARIATION: {file2['file_name']} (similarity: {similarity:.1%})")
    ...
    def generate_recommendations(self, redundant_files: List[Dict]) -> List[Dict]:
        ...
        remove_files = [f for f in redundant_files if f["recommendation"] == "REMOVE_REDUNDANT"]
        review_files = [f for f in redundant_files if f["recommendation"] == "REVIEW_VARIATION"]
    ...
    def execute_cleanup_analysis(self):
        ...
        try:
            ...
            print(f"[BAR_CHART] Found {len(python_files)} Python files in staging root")
    ...
    def save_analysis_results(self):
        ...
        try:
            results_file = self.sandbox_root / f"STAGING_ROOT_REDUNDANCY_ANALYSIS_{self.session_id}.json"
    ...
    def generate_summary_report(self):
        ...
        # Summary statistics
        remove_count = len([r for r in recommendations if r["action"] == "REMOVE"])
        review_count = len([r for r in recommendations if r["action"] == "REVIEW"])
        safe_remove_count = len([r for r in recommendations if r.get("safe_to_remove", False)])
        
        print(f"[BAR_CHART] Analysis Summary:")
        print(f"  [?] Total files analyzed: {self.analysis_results['total_files']}")
        print(f"  [?] File groups identified: {len(self.analysis_results['file_groups'])}")
        ...
        total_size = sum(Path(r["file_path"]).stat().st_size for r in recommendations if r["action"] == "REMOVE")
        ...
        print(f"[CLIPBOARD] NEXT STEPS: Review {review_count} variation files for consolidation")
