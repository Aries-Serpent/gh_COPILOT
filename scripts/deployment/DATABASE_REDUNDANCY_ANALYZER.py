...
class DatabaseRedundancyAnalyzer:
    ...
    def generate_recommendations(self) -> list:
        ...
        if not comparison["can_compare"]:
            recommendations.append(]
                "Cannot analyze - staging or local directory missing")
            return recommendations

        # Recommend removal of identical files from staging
        if comparison["identical_files"]:
            recommendations.append(]
                f"SAFE TO REMOVE: {len(comparison['identical_files'])} identical files from staging")
            for file in comparison["identical_files"]:
                self.analysis_results["safe_to_remove"].append(]
                    file["staging_path"])

        # Analyze staging-only files
        if comparison["staging_only"]:
            recommendations.append(]
                f"REVIEW NEEDED: {len(comparison['staging_only'])} staging-only files")
            ...
            for file_name in comparison["staging_only"]:
                ...
                if days_old > 7:  # Consider files older than 7 days as potential cleanup candidates
                    recommendations.append(]
                        f"CANDIDATE FOR REMOVAL: {file_name} (age: {days_old} days)")
        ...
        if local_analysis["exists"] and local_analysis["total_files"] > 0:
            recommendations.append(]
                "[SUCCESS] Local database appears complete - staging files may be redundant")

        # Storage optimization
        staging_size = self.analysis_results["staging_analysis"].get(]
            "total_size_mb", 0)
        if staging_size > 50:  # If staging is > 50MB
            recommendations.append(]
                f"STORAGE OPTIMIZATION: Staging directory uses {staging_size:.2f} MB")
    ...
    def create_cleanup_plan(self) -> dict:
        ...
        if staging_analysis["exists"]:
            for file_path in cleanup_plan["safe_removals"]
                + cleanup_plan["redundant_removals"]:
                file_name = Path(file_path).name
                if file_name in staging_analysis["files_by_name"]:
                    cleanup_plan["space_savings_mb"] += staging_analysis["files_by_name"][file_name]["size"] / (]
                        1024 * 1024)
...
def main():
    ...
    print(
        f"[BAR_CHART] Total recommendations: {len(results['recommendations'])}")
    print(
        f"[BAR_CHART] Files safe to remove: {len(results['safe_to_remove'])}")
    print(
        f"[BAR_CHART] Redundant files identified: {len(results['redundant_files'])}")
