...
def multi_template_demo():
    ...
    demo_configs = [
        {
            "template": "enterprise_utility_script",
            "env": "development",
            "name": "utility_demo.py",
            "class": "UtilityDemo",
            "description": "Enterprise utility script for demonstrations"
        },
        {
            "template": "enterprise_validation_script", 
            "env": "production",
            "name": "validation_demo.py",
            "class": "ValidationDemo",
            "description": "Enterprise validation script for compliance checking"
        },
        {
            "template": "utility_template",
            "env": "staging",
            "name": "basic_utility_demo.py", 
            "class": "BasicUtilityDemo",
            "description": "Basic utility template demonstration"
        }
    ]
    ...
    for i, config in enumerate(demo_configs, 1):
        ...
        if result["status"] == "success":
            ...
            if "UNIQUE constraint failed: generated_scripts.content_hash" in error_msg:
                print("   [?][?]  This indicates content deduplication is working correctly!")
                print("   [?][?]  The platform prevents duplicate script generation.")
    
    print(f"\n[TARGET] DEMONSTRATION SUMMARY")
    print("=" * 40)
    print(f"Templates Tested: {len(demo_configs)}")
    print(f"Successful Generations: {successful_generations}")
    print(f"Success Rate: {(successful_generations/len(demo_configs)*100):.1f}%")
