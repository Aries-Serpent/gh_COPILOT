...
class DeploymentStatusChecker:
    ...
    def __init__(self):
        ...
        print(
            f"[TIME] Check Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def check_file_status(self):
        ...
        for file_path in critical_files:
            ...
            if full_path.exists():
                ...
                print(
                    f"[SUCCESS] {file_path} - {file_status[file_path]['size']} bytes")
            else:
    ...
    def generate_quick_start_commands(self):
        ...
        commands_file = self.workspace / \
            f"immediate_activation_commands_{self.timestamp}.txt"
    ...
    def run_status_check(self):
        ...
        try:
            ...
            summary_file = self.workspace / \
                f"deployment_status_summary_{self.timestamp}.json"
            ...
            print(
                f"   [?] Critical Files: {sum(1 for f in file_status.values() if f.get('exists', False))}/{len(file_status)}")
