...
class StepFrameworkAnalyzer:
    ...
    def analyze_step_files(self):
        ...
        # Find step files
        step_files = [
            self.staging_root / "step1_factory_deployment.py",
            self.staging_root / "step2_monitor_learning.py", 
            self.staging_root / "step3_collect_analytics.py",
            self.staging_root / "step4_performance_analysis.py",
            self.staging_root / "step5_scale_capabilities.py",
            self.staging_root / "step6_continuous_innovation.py"
        ]
            ...
            for line in lines:
                ...
                if any(keyword in line_clean for keyword in ['autonomous', 'enterprise', 'framework', 'deployment', 'monitoring']):
                    if len(line.strip()) > 20 and not line.strip().startswith('#'):
                        key_features.append(line.strip()[:80] + '...' if len(line.strip()) > 80 else line.strip())
    ...
    def check_redundancy_patterns(self, file_details: List[Dict]):
        ...
        if duplicate_functions:
            print(f"[BAR_CHART] Found {len(duplicate_functions)} functions appearing in multiple files:")
            for func_name, files in list(duplicate_functions.items())[:10]:  # Show top 10
        ...
        for details in file_details:
            if not details.get('analysis_failed'):
                for cls in details.get('classes', []):
                    cls_name = cls.strip().split('(')[0].replace('class ', '').replace(':', '')
        ...
        if duplicate_classes:
            print(f"[BAR_CHART] Found {len(duplicate_classes)} classes appearing in multiple files:")
            for cls_name, files in list(duplicate_classes.items())[:5]:  # Show top 5
    ...
    def generate_step_recommendations(self, file_details: List[Dict]):
        ...
        total_size = sum(details.get('size', 0) for details in file_details if not details.get('analysis_failed'))
        
        print(f"[BAR_CHART] Step Framework Summary:")
        print(f"  [?] Total files: {len(file_details)}")
        print(f"  [?] Total size: {total_size:,} bytes ({total_size/1024:.1f} KB)")
        print(f"  [?] Average size: {total_size/len(file_details):,.0f} bytes per file")
        
        # Analyze file usage patterns
        print(f"\n[SEARCH] USAGE ANALYSIS:")
        
        # Check modification dates
        recent_files = [d for d in file_details if not d.get('analysis_failed')]
        ...
        print(f"  [SUCCESS] KEEP ALL STEP FILES - Each represents a distinct deployment phase")
        print(f"  [CLIPBOARD] All files are unique and serve different purposes in the framework")
        print(f"  [WRENCH] No redundancy detected - files have different functions and classes")
        print(f"  [STORAGE] Total space usage ({total_size/1024:.1f} KB) is reasonable for framework completeness")
        
        # Check if any files are particularly large or small
        avg_size = total_size / len(file_details)
        large_files = [d for d in file_details if d.get('size', 0) > avg_size * 1.5]
        small_files = [d for d in file_details if d.get('size', 0) < avg_size * 0.5]
        
        if large_files:
            print(f"\n[BAR_CHART] NOTABLY LARGE FILES:")
            for details in large_files:
                print(f"  [?] {details['filename']}: {details['size']:,} bytes")
    ...
    def check_database_coverage(self):
        ...
        try:
            ...
            if database_path.exists():
                ...
                print(f"[RULER] Database size: {database_path.stat().st_size:,} bytes")
                ...
                print(f"[CLIPBOARD] However, step framework files are still needed for:")
