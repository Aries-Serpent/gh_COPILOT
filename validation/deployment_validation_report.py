...
def validate_deployment_structure():
    ...
    for db_file in db_files:
        try:
            ...
            print(f"✓ {db_file.name:<30} ({len(tables)} tables, {db_file.stat().st_size} bytes)")
    ...
    validation_results["script_validation"]["total_scripts"] = len(script_files)
    ...
    print(f"✓ Average Script Size: {sum(f.stat().st_size for f in script_files) / len(script_files):.0f} bytes")
    print()
    ...
    # Summary Statistics
    total_files = sum(len(list(d.rglob("*"))) for d in base_path.iterdir() if d.is_dir())
