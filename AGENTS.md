...
* **Artifact Management**: Runtime directories such as `builds/`, `logs/`, `results/`, `reports/`, and `tmp/` must not be committed. Store temporary outputs under `artifacts/` or archive them with `artifact_manager.py` when needed.
