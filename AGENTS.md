...
* **Artifact Management**: Place runtime and build outputs such as `artifacts/builds/`, `artifacts/results/`, `artifacts/logs/`, and `artifacts/reports/` under the `artifacts/` directory and ensure these paths are ignored by Git. This keeps the repository root clean and avoids committing transient files.
* **Wrap-Up Process**: Ensure `scripts/wlc_session_manager.py` runs at workflow exit points so sessions are logged to `production.db`, recorded in `artifacts/logs/`, and mirrored to `$GH_COPILOT_BACKUP_ROOT/logs`.
* **Mandatory Invocation**: Orchestrators and CLI utilities must invoke `scripts/wlc_session_manager.py --orchestrate` on shutdown (for example, in a `finally` block) to guarantee wrap-up logging.
...
- Use `scripts/wlc_session_manager.py` to record wrap-up sessions. Ensure it
  creates a row in `production.db`, writes a log to `artifacts/logs/`, and mirrors it under `$GH_COPILOT_BACKUP_ROOT/logs`.
