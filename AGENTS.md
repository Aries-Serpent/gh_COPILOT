...
* **Artifact Management**: Place runtime and build outputs such as `builds/`, `results/`, `logs/`, and `reports/` under the `artifacts/` directory and ensure these paths are ignored by Git. This keeps the repository root clean and avoids committing transient files.
* **Wrap-Up Process**: Ensure `scripts/wlc_session_manager.py` runs at workflow exit points so sessions are logged to `production.db` and `$GH_COPILOT_BACKUP_ROOT`.
