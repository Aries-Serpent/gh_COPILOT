...
* **Artifact Management**: Place build outputs, logs, reports (including coverage), and other temporary files under the `artifacts/` directory and ensure these paths are ignored by Git. Legacy artifacts that were checked in should be moved to `archive/` or removed to keep the repository root clean.
