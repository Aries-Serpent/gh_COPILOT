...
* **Hard Output Limit**: The Codex terminal enforces a **1600-byte** maximum per line. Always pipe potentially long output through `clw` (e.g., `command | clw`) or redirect to a log file and read it in chunks. Set `CLW_MAX_LINE_LENGTH` to **1550** or less so wrapped output never exceeds the limit.
