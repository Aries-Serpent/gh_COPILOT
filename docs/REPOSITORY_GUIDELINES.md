...
This project uses a single `AGENTS.md` at the repository root as the authoritative guide for all contributors. A copy is kept at `.github/instructions/AGENTS.md` for reference. Any older guides are stored in the `archives/` directory.
...
If any appear in `archive*/` or `archives/`, move them to a manual cleanup folder outside the repository for removal. Once cleared, AGENTS should extend
