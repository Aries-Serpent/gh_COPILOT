#!/usr/bin/env python3
"""
🔍 Universal Base64 Decoder and File Analyzer
Decodes base64 content and analyzes what type of file it is
"""

import base64
import zipfile
import os
import tempfile
from pathlib import Path
from datetime import datetime

def analyze_base64_content(base64_string: str) -> dict:
    """
    🔍 Analyze base64 content to determine file type and structure
    
    Args:
        base64_string: The base64-encoded content
    
    Returns:
        dict: Analysis results with file type and metadata
    """
    
    print("🔍 Universal Base64 Content Analyzer")
    print("=" * 50)
    print(f"📊 Input size: {len(base64_string):,} characters")
    
    try:
        # Step 1: Decode base64 to bytes
        print("🔄 Step 1: Decoding base64...")
        decoded_bytes = base64.b64decode(base64_string)
        print(f"✅ Decoded {len(decoded_bytes):,} bytes")
        
        # Step 2: Analyze file signature/magic bytes
        print("🔄 Step 2: Analyzing file signature...")
        
        # Check first few bytes for common file signatures
        first_bytes = decoded_bytes[:16]
        hex_signature = first_bytes.hex().upper()
        print(f"📝 First 16 bytes (hex): {hex_signature}")
        print(f"📝 First 16 bytes (ascii): {repr(first_bytes)}")
        
        # Common file signatures
        file_signatures = {
            b'PK\x03\x04': 'ZIP Archive',
            b'PK\x05\x06': 'ZIP Archive (empty)',
            b'PK\x07\x08': 'ZIP Archive (spanned)',
            b'\x50\x4B\x03\x04': 'ZIP Archive',
            b'\x1f\x8b': 'GZIP Archive',
            b'\x42\x5a\x68': 'BZIP2 Archive',
            b'\x37\x7a\xbc\xaf\x27\x1c': '7Z Archive',
            b'Rar!': 'RAR Archive',
            b'\x89PNG': 'PNG Image',
            b'\xff\xd8\xff': 'JPEG Image',
            b'GIF8': 'GIF Image',
            b'\x00\x00\x00\x20\x66\x74\x79\x70': 'MP4 Video',
            b'%PDF': 'PDF Document',
            b'\xd0\xcf\x11\xe0': 'Microsoft Office Document',
        }
        
        detected_type = "Unknown Binary File"
        for signature, file_type in file_signatures.items():
            if decoded_bytes.startswith(signature):
                detected_type = file_type
                break
        
        print(f"🎯 Detected file type: {detected_type}")
        
        # Step 3: Try to detect if it's text content
        is_text = False
        text_content = None
        try:
            text_content = decoded_bytes.decode('utf-8')
            is_text = True
            print("📄 Content appears to be UTF-8 text")
        except UnicodeDecodeError:
            try:
                text_content = decoded_bytes.decode('latin-1')
                print("📄 Content might be Latin-1 encoded text")
            except:
                print("📄 Content is binary data")
        
        # Step 4: Create output file
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        if detected_type == 'ZIP Archive':
            output_filename = f"decoded_archive_{timestamp}.zip"
        elif 'Image' in detected_type:
            ext = detected_type.split()[0].lower()
            output_filename = f"decoded_image_{timestamp}.{ext}"
        elif detected_type == 'PDF Document':
            output_filename = f"decoded_document_{timestamp}.pdf"
        elif is_text:
            output_filename = f"decoded_text_{timestamp}.txt"
        else:
            output_filename = f"decoded_binary_{timestamp}.bin"
        
        # Write decoded content to file
        output_path = Path(output_filename)
        with open(output_path, 'wb') as f:
            f.write(decoded_bytes)
        
        print(f"💾 Saved decoded content to: {output_path}")
        
        # Step 5: Additional analysis for specific file types
        additional_info = {}
        
        if detected_type == 'ZIP Archive':
            try:
                print("🔄 Step 3: Analyzing ZIP contents...")
                with zipfile.ZipFile(output_path, 'r') as zip_ref:
                    zip_info = zip_ref.namelist()
                    additional_info['zip_contents'] = zip_info
                    print(f"📦 ZIP contains {len(zip_info)} files:")
                    for file in zip_info:
                        print(f"   📄 {file}")
            except Exception as e:
                print(f"⚠️ ZIP analysis failed: {e}")
        
        elif is_text and text_content:
            print("🔄 Step 3: Analyzing text content...")
            lines = text_content.split('\n')
            additional_info['text_lines'] = len(lines)
            additional_info['text_preview'] = text_content[:500] + "..." if len(text_content) > 500 else text_content
            print(f"📄 Text content ({len(lines)} lines):")
            print("--- Preview (first 500 chars) ---")
            print(additional_info['text_preview'])
            print("--- End Preview ---")
        
        return {
            'success': True,
            'file_type': detected_type,
            'size': len(decoded_bytes),
            'hex_signature': hex_signature,
            'output_file': str(output_path),
            'is_text': is_text,
            'additional_info': additional_info
        }
        
    except Exception as e:
        return {
            'success': False,
            'error': str(e),
            'error_type': type(e).__name__
        }

if __name__ == "__main__":
    # Your base64 content
    base64_content = """"""
    
    # Analyze the content
    result = analyze_base64_content(base64_content)
    
    if result['success']:
        print(f"\n✅ Analysis complete!")
        print(f"📊 File type: {result['file_type']}")
        print(f"💾 Size: {result['size']:,} bytes")
        print(f"📁 Saved to: {result['output_file']}")
    else:
        print(f"\n❌ Analysis failed: {result['error']}")
