#!/usr/bin/env python3
"""
🔧 Advanced Base64 and Binary File Analyzer
Performs deep analysis of base64 content including hex dump and structure analysis
"""

import base64
import struct
from datetime import datetime
from pathlib import Path

def hex_dump(data: bytes, start_offset: int = 0, max_lines: int = 20) -> str:
    """Create a hex dump of binary data"""
    lines = []
    for i in range(0, min(len(data), max_lines * 16), 16):
        hex_part = ' '.join(f'{b:02x}' for b in data[i:i+16])
        ascii_part = ''.join(chr(b) if 32 <= b <= 126 else '.' for b in data[i:i+16])
        lines.append(f'{start_offset + i:08x}: {hex_part:<48} |{ascii_part}|')
    return '\n'.join(lines)

def analyze_zip_structure(data: bytes) -> dict:
    """Analyze ZIP file structure and attempt repair"""
    
    analysis = {
        'has_local_headers': [],
        'has_central_directory': False,
        'has_end_record': False,
        'file_entries': [],
        'errors': []
    }
    
    try:
        # Look for local file headers (PK\x03\x04)
        pos = 0
        while pos < len(data):
            if data[pos:pos+4] == b'PK\x03\x04':
                print(f"🔍 Found local file header at offset {pos:08x}")
                
                try:
                    # Parse local file header
                    header = struct.unpack('<HHHHHHIIIHH', data[pos+4:pos+30])
                    version, flags, method, time, date, crc32, comp_size, uncomp_size, name_len, extra_len = header
                    
                    filename_start = pos + 30
                    filename_end = filename_start + name_len
                    filename = data[filename_start:filename_end].decode('utf-8', errors='ignore')
                    
                    file_data_start = filename_end + extra_len
                    file_data_end = file_data_start + comp_size
                    
                    print(f"   📄 Filename: {filename}")
                    print(f"   📊 Compressed size: {comp_size}")
                    print(f"   📊 Uncompressed size: {uncomp_size}")
                    print(f"   🔧 Compression method: {method}")
                    
                    analysis['file_entries'].append({
                        'filename': filename,
                        'compressed_size': comp_size,
                        'uncompressed_size': uncomp_size,
                        'method': method,
                        'data_start': file_data_start,
                        'data_end': file_data_end
                    })
                    
                    pos = file_data_end
                    
                except Exception as e:
                    analysis['errors'].append(f"Error parsing header at {pos:08x}: {e}")
                    pos += 4
            else:
                pos += 1
        
        # Look for central directory signature (PK\x01\x02)
        if b'PK\x01\x02' in data:
            analysis['has_central_directory'] = True
            print("🔍 Found central directory signature")
        
        # Look for end of central directory signature (PK\x05\x06)
        if b'PK\x05\x06' in data:
            analysis['has_end_record'] = True
            print("🔍 Found end of central directory signature")
            
    except Exception as e:
        analysis['errors'].append(f"General analysis error: {e}")
    
    return analysis

def extract_files_manually(data: bytes, analysis: dict) -> dict:
    """Manually extract files from ZIP data"""
    
    extracted = {
        'success': False,
        'files': [],
        'errors': []
    }
    
    try:
        output_dir = Path(f"manual_extraction_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        output_dir.mkdir(exist_ok=True)
        
        for entry in analysis['file_entries']:
            try:
                print(f"🔧 Manually extracting: {entry['filename']}")
                
                # Extract compressed data
                compressed_data = data[entry['data_start']:entry['data_end']]
                
                # Try to decompress if method is deflate (8)
                if entry['method'] == 8:  # DEFLATE
                    import zlib
                    try:
                        decompressed = zlib.decompress(compressed_data, -15)  # Raw deflate
                        print(f"   ✅ Decompressed {len(compressed_data)} -> {len(decompressed)} bytes")
                    except Exception:
                        print("   ⚠️ Decompression failed, saving raw data")
                        decompressed = compressed_data
                elif entry['method'] == 0:  # STORED (no compression)
                    decompressed = compressed_data
                    print(f"   ✅ No decompression needed (stored)")
                else:
                    print(f"   ⚠️ Unknown compression method {entry['method']}, saving raw data")
                    decompressed = compressed_data
                
                # Save file
                safe_filename = entry['filename'].replace('/', '_').replace('\\', '_')
                output_path = output_dir / safe_filename
                
                with open(output_path, 'wb') as f:
                    f.write(decompressed)
                
                extracted['files'].append({
                    'original_name': entry['filename'],
                    'extracted_path': str(output_path),
                    'size': len(decompressed)
                })
                
                print(f"   💾 Saved to: {output_path}")
                
            except Exception as e:
                error_msg = f"Failed to extract {entry['filename']}: {e}"
                extracted['errors'].append(error_msg)
                print(f"   ❌ {error_msg}")
        
        extracted['success'] = len(extracted['files']) > 0
        extracted['output_directory'] = str(output_dir)
        
    except Exception as e:
        extracted['errors'].append(f"Manual extraction failed: {e}")
    
    return extracted

def deep_analyze_base64(base64_string: str) -> dict:
    """Perform deep analysis of base64 content"""
    
    print("🔍 Deep Base64 Analysis")
    print("=" * 50)
    
    try:
        # Decode base64
        data = base64.b64decode(base64_string)
        print(f"📊 Decoded {len(data):,} bytes")
        
        # Show hex dump of first part
        print("\n📋 HEX DUMP (first 320 bytes):")
        print(hex_dump(data[:320]))
        
        # Analyze as ZIP
        print(f"\n🔍 ZIP Structure Analysis:")
        zip_analysis = analyze_zip_structure(data)
        
        # Manual extraction attempt
        if zip_analysis['file_entries']:
            print(f"\n🔧 Manual File Extraction:")
            extraction = extract_files_manually(data, zip_analysis)
            
            return {
                'success': True,
                'data_size': len(data),
                'zip_analysis': zip_analysis,
                'extraction': extraction
            }
        else:
            print("❌ No valid file entries found")
            return {
                'success': False,
                'data_size': len(data),
                'zip_analysis': zip_analysis,
                'error': 'No valid ZIP entries found'
            }
            
    except Exception as e:
        return {
            'success': False,
            'error': str(e)
        }

if __name__ == "__main__":
    base64_content = """"""
    
    result = deep_analyze_base64(base64_content)
    
    if result['success']:
        print(f"\n🎉 SUCCESS!")
        if 'extraction' in result and result['extraction']['success']:
            print(f"📁 Extracted {len(result['extraction']['files'])} files")
            print(f"📂 Output directory: {result['extraction']['output_directory']}")
    else:
        print(f"\n❌ FAILED: {result.get('error', 'Unknown error')}")
