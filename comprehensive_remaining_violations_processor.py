                    ...
                    try:
                        ...
                        if error_code in self.supported_violations:
                            ...
                            if fixed_content != modified_content:
                                ...
                                logger.debug(f"No change applied for {error_code} at {file_path}:{violation['line_number']}")
                        else:
                            batch_stats['failed'] += 1
                            
                    except Exception as e:
                        batch_stats['failed'] += 1
                        logger.error(f"❌ Failed to fix {violation['error_code']} in {file_path}:{violation['line_number']}: {e}")
