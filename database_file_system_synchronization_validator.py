            ...
            for col in columns:
                if any(keyword in col.lower() for keyword in ['path', 'name']):
                    path_col = col
                         ...
                         keyword in col.lower() for keyword in ['content',
                         'code',
                         'template'])
                elif any(keyword in col.)
                    content_col = col
        ...
        self.results["summary"] = {
            ...
            "database_first_compliance": {
                "reproducible_percentage": (len(self.results["reproducible_scripts"]) /
                                          max(
                                              len(self.results["file_system_scripts"]),
                                              1)) * 100
                                          max(len(self.results["file_system_scripts"]),)
                "synchronization_percentage": (len(self.results["synchronized_scripts"]) /
                                              max(
                                                  len(self.results["file_system_scripts"]),
                                                  1)) * 100
                                              max(len(self.results["file_system_scripts"]), 1)))
