...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'enterprise_script_sync_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8')
    ]
)
            ...
            with sqlite3.connect(self.main_db_path) as conn:
                ...
                if existing:
                    ...
                    if existing_hash != script_info['content_hash']:
                        ...
                        (script_id, content_text, content_hash, archived_timestamp, archive_reason)
                        VALUES (?, ?, ?, ?, ?)
                ...
                for script_type, count, avg_size in patterns:
                    ...
                    if template_content:
                        ...
                        """, (f"{script_type}_standard_template", script_type,
                              template_content, json.dumps({"count": count, "avg_size": avg_size}),
...
'''
        ...
        return templates.get(script_type, f"# Template for {script_type}\n# {{IMPLEMENTATION_PLACEHOLDER}}")
        ...
        try:
            with sqlite3.connect(self.main_db_path) as conn:
                ...
                self.sync_results["compliance_metrics"] = {
                    ...
                    "enterprise_compliance_score": min(95, (db_script_count / total_filesystem_scripts) * 100) if total_filesystem_scripts else 0,
        ...
        try:
            ...
            full_report = {
                ...
                "compliance_assessment": {
                    "status": "ENHANCED" if self.sync_results["compliance_metrics"].get("enterprise_compliance_score", 0) >= 90 else "NEEDS_IMPROVEMENT",
