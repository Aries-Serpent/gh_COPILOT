...
def decode_and_extract_base64_zip(base64_string: str, output_dir: str = None) -> dict:
    ...
    print("🚀 Base64 ZIP Decoder Starting...")
    ...
    try:
        ...
        print(f"✅ Decoded {len(zip_bytes):,} bytes")
        
        # Step 2: Create temporary file for ZIP
        with tempfile.NamedTemporaryFile(suffix='.zip', delete=False) as temp_zip:
            temp_zip.write(zip_bytes)
            temp_zip_path = temp_zip.name
        
        print(f"📁 Temporary ZIP file: {temp_zip_path}")
        
        # Step 3: Set up extraction directory
        ...
        print(f"🔄 Step 2: Extracting to {output_path}...")
        extracted_files = []
...
def analyze_extracted_files(results: dict):
    ...
    if not results['success']:
        print(f"❌ Cannot analyze - extraction failed: {results['error']}")
        return
...
if __name__ == "__main__":
    ...
    print("🔓 Base64 ZIP Decoder Utility")
    print("=" * 50)
    ...
    if results['success']:
        print(f"\n✅ SUCCESS: Files extracted to '{results['output_directory']}'")
        print(f"📊 Summary: {results['total_files']} files, {results['total_size']:,} bytes total")
    else:
