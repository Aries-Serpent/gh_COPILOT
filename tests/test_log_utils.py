...
def test_log_event_multiple_tables(tmp_path, monkeypatch, mode):
    ...
    assert _log_event(
        {"event": "fail", "module": "m", "level": "INFO", "target": "t1", "details": "d"},
        table="rollback_failures",
        db_path=db,
        test_mode=mode,
    )
    assert _log_event(
        {"event": "fail2", "module": "m", "level": "INFO", "target": "t2", "details": "d2"},
        table="rollback_failures",
        db_path=db,
        test_mode=mode,
    )
    assert _log_event(
        {"event": "fail3", "module": "m", "level": "INFO", "target": "t3", "details": "d3"},
        table="rollback_failures",
        db_path=db,
        test_mode=mode,
    )
