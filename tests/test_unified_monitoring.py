...
def test_collect_metrics(tmp_path, monkeypatch):
    monkeypatch.setattr(umos_module.psutil, "cpu_percent", lambda interval=1: 10.0)
    monkeypatch.setattr(umos_module.psutil, "virtual_memory", lambda: SimpleNamespace(percent=20.0))
    monkeypatch.setattr(umos_module.psutil, "disk_usage", lambda p: SimpleNamespace(percent=30.0))
    monkeypatch.setattr(umos_module.psutil, "net_io_counters", lambda: SimpleNamespace(bytes_sent=100, bytes_recv=200))
    monkeypatch.setattr(umos_module.psutil, "pids", lambda: [1, 2, 3])

    system = umos_module.UnifiedMonitoringOptimizationSystem(workspace_root=str(tmp_path))
    ...
    count = system.metrics_db.execute("SELECT COUNT(*) FROM performance_metrics").fetchone()[0]
    assert count == 1
