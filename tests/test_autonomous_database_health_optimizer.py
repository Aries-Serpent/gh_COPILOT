...
class TestAutonomousDatabaseHealthOptimizer:
    ...
    def test_no_duplicate_method_definitions(self, optimizer):
        ...
        assert len(learning_methods) == 1, f"Expected 1 learning pattern method, found {len(learning_methods)}: {learning_methods}"
        ...
        assert len(history_methods) == 1, f"Expected 1 optimization history method, found {len(history_methods)}: {history_methods}"
        assert '_load_optimization_history' in history_methods, "Should have consolidated _load_optimization_history method"
    ...
    def test_missing_strategy_implementation(self, optimizer):
        ...
        assert integrity_strategy['sql_commands'] == expected_commands, f"Unexpected SQL commands: {integrity_strategy['sql_commands']}"
        
        # Check expected improvement
        assert integrity_strategy['expected_improvement'] == 15.0, f"Unexpected improvement value: {integrity_strategy['expected_improvement']}"
    ...
    def test_configuration_initialization_not_duplicated(self, optimizer):
        ...
        assert optimizer.optimization_strategies == expected_strategies, "Optimization strategies not configured correctly"
