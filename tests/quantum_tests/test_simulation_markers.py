...
from ghc_quantum.algorithms.hardware_aware import HardwareAwareAlgorithm
from ghc_quantum.quantum_compliance_engine import QuantumComplianceEngine, WORKSPACE_ROOT
...
def test_get_backend_warns_when_aer_missing(monkeypatch):
    ...
    monkeypatch.setattr("ghc_quantum.utils.backend_provider.Aer", None)
    # Combine: use the most informative ImportError message ("mock" preferred for clarity)
    monkeypatch.setattr(
        "ghc_quantum.utils.backend_provider._AER_IMPORT_ERROR",
        ImportError("mock"),
        raising=False,
    )
...
def test_hardware_aware_algorithm_warns_no_backend(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "ghc_quantum.algorithms.hardware_aware.get_backend", lambda **_: None
    )
...
def test_compliance_engine_warns_without_qiskit(monkeypatch, caplog):
    ...
    monkeypatch.setattr("ghc_quantum.quantum_compliance_engine.QISKIT_AVAILABLE", False)
    with caplog.at_level(logging.WARNING):
