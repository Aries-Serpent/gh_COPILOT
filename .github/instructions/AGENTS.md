...
* **Hard Output Limit**: The terminal resets if any line exceeds **1600 bytes**. Always pipe large or uncertain output through `clw` (`command | clw`) or redirect to a file and inspect in smaller chunks. You can adjust the wrap threshold via `CLW_MAX_LINE_LENGTH` (default 1550).
