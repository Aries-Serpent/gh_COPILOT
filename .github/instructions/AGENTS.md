...
* **Primary Instructions**: The root `AGENTS.md` serves as the main reference for agent behavior. Supplemental `.instructions.md` modules located in `.github/instructions/` provide topic-specific guidance. Consult those files along with this document and the user’s prompt.
...
- Use `scripts/wlc_session_manager.py` to log wrap-up sessions, ensuring entries
  in `production.db` and logs under `$GH_COPILOT_BACKUP_ROOT/logs`.
