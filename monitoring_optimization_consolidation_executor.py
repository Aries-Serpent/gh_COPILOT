...
MANIFEST_FILE = WORKSPACE_ROOT / "monitoring_optimization_consolidation_manifest.json"
...
class MonitoringOptimizationConsolidationExecutor:
    ...
    def discover_legacy_files(self) -> List[Path]:
        ...
        for pattern in self.discovery_patterns:
            matched_files = [Path(p) for p in glob.glob(str(WORKSPACE_ROOT / pattern), recursive=True)]
    ...
    def archive_legacy_files(self) -> None:
        ...
        logger.info(f"Archived {len(self.archived_files)} files to {self.backup_dir}")
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        logger.info("Starting Monitoring & Optimization Consolidation (Phase 6)...")
        ...
        try:
            ...
            logger.info("Monitoring & Optimization Consolidation (Phase 6) complete!")
            return {
                "success": True,
                "manifest": manifest
            }
...
def main():
    ...
    if result["success"]:
        ...
        print("=" * 80)
        print(f"Discovered: {result['manifest']['statistics']['discovered_files']} files")
        print(f"Archived: {result['manifest']['statistics']['archived_files']} files")
        print(f"Removed: {result['manifest']['statistics']['removed_files']} files")
